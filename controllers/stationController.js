...
const announce_fuel_queue = async (req, res) => {
  ...
  try {
    let result1 = await stationDBHelper.updateLastAnnounced(
      regNo,
      ftype,
      lastAnnounced
    ...
    let result2 = await queueDBHelper.addNewAnnouncedQueue(
      regNo,
      ftype,
      reqs,
      stime,
...
const get_announced_queues = async (req, res) => {
  ...
  try {
    let queues = await queueDBHelper.findQueuesByStRegNo(regNo, [
      "announced",
      "active",
    ]);
...
const update_queue = async (req, res) => {
  ...
  try {
    //handle any possible errors
    let result = await queueDBHelper.updateQueue(id, state, estEndTime);
    //return necessary data
    res.json({
      status: "ok",
      state: result.state,
    ...
    if (queue) {
      ...
      if (queue_requests.length > 0) {
        ...
        for (let i = 0; i < queue_requests.length; i++) {
          if (
            ((queue_requests[i].registrationNo && queue_requests[i].registrationNo === regNo) || queue_requests[i].userID === regNo) &&
            queue_requests[i].fuelType === fuelType &&
            queue_requests[i].state !== "closed"
          ) {
        ...
        if (selected_req) {
          ...
          let req_id = selected_req._id.toString();
          ...
          if (user) {
            ...
            if (userType === "personal") {
              ...
              await orgDBHelper.updateFillingDetails(
                regNo,
                currLastFilledDate,
                currRemainingQuotas
              );
...
//get active queues
const get_active_queues = async (req, res) => {
    let regNo = req.params.regNo;
    ...
    try {
        ...
        for (let i = 0; i < queues.length; i++) {
          ...
          for (let j = 0; j < req_arr.length; j++) {
            ...
            if (reqest.registrationNo) {
              temp["registrationNo"] = reqest.registrationNo;
              reqests[reqest.registrationNo] = temp;
            }else{
                reqests[reqest.userID] = temp;
            } 
          }
          let a = queue.toJSON();
          a.requests = reqests;
